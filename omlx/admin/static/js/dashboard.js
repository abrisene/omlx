    ...
    function dashboard() {
        return {
            ...
            globalSettings: {
                ...
                scheduler: { max_concurrent_requests: 8, prefill_batch_size: 1, prefill_step_size: 2048 },
            ...
            modelSettings: {
                ...
                sampler_priority_text: '',
                dry_multiplier: null,
                dry_base: 1.75,
                dry_allowed_length: 2,
                dry_sequence_breakers_text: '',
            ...
            async saveGlobalSettings() {
                ...
                if (!s.scheduler.prefill_step_size) errors.push('Prefill Step Size');
                ...
                try {
                    const response = await fetch('/admin/api/global-settings', {
                        ...
                        body: JSON.stringify({
                            ...
                            prefill_step_size: this.globalSettings.scheduler.prefill_step_size,
            ...
            async openModelSettings(model) {
                ...
                this.modelSettings = {
                    ...
                    top_n_sigma: settings.top_n_sigma ?? null,
                    min_k: settings.min_k ?? null,
                    dynamic_temperature: !!settings.dynamic_temperature,
                    dynatemp_low: settings.dynatemp_low ?? 0.7,
                    dynatemp_high: settings.dynatemp_high ?? 1.2,
                    dynatemp_exponent: settings.dynatemp_exponent ?? 1.0,
                    temperature_last: !!settings.temperature_last,
                    sampler_priority_text: Array.isArray(settings.sampler_priority) ? settings.sampler_priority.join('\n') : '',
                    dry_multiplier: settings.dry_multiplier ?? null,
                    dry_base: settings.dry_base ?? 1.75,
                    dry_allowed_length: settings.dry_allowed_length ?? 2,
                    dry_sequence_breakers_text: Array.isArray(settings.dry_sequence_breakers) ? settings.dry_sequence_breakers.join(', ') : '',
                    ...
                    const response = await fetch(`/admin/api/models/${encodeURIComponent(this.selectedModel.id)}/settings`, {
                        ...
                        body: JSON.stringify((() => {
                            ...
                            return {
                                ...
                                top_n_sigma: Number.isFinite(this.modelSettings.top_n_sigma) ? this.modelSettings.top_n_sigma : null,
                                min_k: Number.isFinite(this.modelSettings.min_k) ? this.modelSettings.min_k : null,
                                dynamic_temperature: !!this.modelSettings.dynamic_temperature,
                                dynatemp_low: Number.isFinite(this.modelSettings.dynatemp_low) ? this.modelSettings.dynatemp_low : null,
                                dynatemp_high: Number.isFinite(this.modelSettings.dynatemp_high) ? this.modelSettings.dynatemp_high : null,
                                dynatemp_exponent: Number.isFinite(this.modelSettings.dynatemp_exponent) ? this.modelSettings.dynatemp_exponent : null,
                                temperature_last: !!this.modelSettings.temperature_last,
                                sampler_priority: this.modelSettings.sampler_priority_text
                                    ? this.modelSettings.sampler_priority_text.split(/[\n,]+/).map(x => x.trim()).filter(Boolean)
                                    : null,
                                ...
                                dry_sequence_breakers: this.modelSettings.dry_sequence_breakers_text
                                    ? this.modelSettings.dry_sequence_breakers_text.split(',').map(x => x.trim()).filter(Boolean)
                                    : null,
